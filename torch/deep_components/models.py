...
class MLPX3(nn.Module):
  def __init__(self, emb_dim, seq_len, device, id_cnt_dict, user_layer=[], item_layer=[], main_layer_sizes = [1024, 768, 768, 1]):
    super(MLPX3, self).__init__()

    self.emb_dim = emb_dim
    self.seq_len = seq_len
    self.device = device
    self.main_layer_sizes = main_layer_sizes
    self.user_layer = user_layer
    self.item_layer = item_layer
    self.user_inputsize = emb_dim * 13
    self.item_inputsize = emb_dim * 8
    ...
    if user_layer != []:
      layers = []
      prev_dim = self.user_inputsize
      for out_dim in self.user_layer:
          layers.extend([
    ...
    if item_layer != []:
      layers = []
      prev_dim = self.item_inputsize
      for out_dim in self.item_layer:
          layers.extend([
              nn.LayerNorm(prev_dim),
              nn.Linear(prev_dim, out_dim, bias=False),
              nn.PReLU()
      ...
      self.photo_encoder = nn.Sequential(*layers)
    if user_layer != []:
      self.user_embeddings = nn.Sequential(
          nn.LayerNorm(self.user_layer[-1]),
          nn.Linear(self.user_layer[-1], main_layer_sizes[0], bias=False),
      )
    else:
      self.user_embeddings = nn.Sequential(
          nn.LayerNorm(self.user_inputsize),
          nn.Linear(self.user_inputsize, main_layer_sizes[0], bias=False),
      )
      
    if item_layer != []:
      self.item_embeddings = nn.Sequential(
          nn.LayerNorm(self.item_layer[-1]),
          nn.Linear(self.item_layer[-1], main_layer_sizes[0], bias=False),
      )
    else:
      self.item_embeddings = nn.Sequential(
          nn.LayerNorm(self.item_inputsize),
          nn.Linear(self.item_inputsize, main_layer_sizes[0], bias=False),
      )

    layers = []
    layers.append(nn.PReLU())
    prev_dim = main_layer_sizes[0]
    for out_dim in self.main_layer_sizes[1:-1]:
        layers.extend([
            nn.LayerNorm(prev_dim),
            nn.Linear(prev_dim, out_dim, bias=False),
            nn.PReLU()
        ...
        prev_dim = out_dim
    layers.append(nn.Linear(prev_dim, self.main_layer_sizes[-1], bias=False))
  ...
  def forward_all_by_rank(self, inputs):
    ...
    if self.user_layer != []:
      ...
      u_out = u_input
    u_out = self.user_embeddings(u_out)
    ...
    def compute_photo_logits(u_input, photo_inputs):
      ...
      p_input = torch.cat([vid_emb, aid_emb, cate_two_emb, cate_one_emb, up_emb,
                           up_wda_emb, up_hou_emb, up_min_emb], dim=2)  # b*p*8d
      # p_input b*p*8d b*p*32 u b*(8+5)d b*32
      b, p, l = p_input.shape

      if self.item_layer != []:
        flattened = p_input.view(b*p,-1)
        p_out = self.photo_encoder(flattened)  # b*p*32
        p_out = p_out.view(b,p,-1)
      else:
        p_out = p_input
      flattened = p_out.view(b*p,-1)
      p_out = self.item_embeddings(flattened)
      p_out = p_out.view(b,p,-1)
      out = p_out+u_expanded # b*p*32
      flattened = out.view(b*p, -1)
      logits = self.mlp(flattened)
      logits = logits.view(b,p,1).squeeze(-1)
      return logits

    # Compute logits for each type of photo inputs
    rank_pos_logits = compute_photo_logits(u_out,rank_pos_photos)  # b*10
    rank_neg_logits = compute_photo_logits(u_out,rank_neg_photos)  # b*10
    coarse_neg_logits = compute_photo_logits(u_out,coarse_neg_photos)  # b*10
    prerank_neg_logits = compute_photo_logits(u_out,prerank_neg_photos)  # b*10
  ...
  def forward_all_by_rerank(self, inputs):
    ...
    # Compute logits for each type of photo inputs
    rerank_pos_logits = compute_photo_logits(u_out,rerank_pos_photos)  # b*10
    rerank_neg_logits = compute_photo_logits(u_out,rerank_neg_photos)  # b*10
    rank_neg_logits = compute_photo_logits(u_out,rank_neg_photos)  # b*10
    coarse_neg_logits = compute_photo_logits(u_out,coarse_neg_photos)  # b*10
    prerank_neg_logits = compute_photo_logits(u_out,prerank_neg_photos)  # b*10
...
class MLPX3withres(nn.Module):
  def __init__(self, emb_dim, seq_len, device, id_cnt_dict, user_layer=[], item_layer=[], main_layer_sizes = [1024, 768, 768, 1]):
    super(MLPX3withres, self).__init__()
    ...
    if user_layer != []:
      ...
      prev_dim = out_dim
      for out_dim in self.user_layer[1:]:
          layers.extend([
              PreNormResidualBlock(prev_dim)
          ])
          prev_dim = out_dim
      self.user_encoder = nn.Sequential(*layers)
    ...
    if item_layer != []:
      ...
      prev_dim = out_dim
      for out_dim in self.item_layer[1:]:
          layers.extend([
              PreNormResidualBlock(prev_dim)
          ])
          prev_dim = out_dim
    ...
    # if self.item_layer != [] and self.user_layer != []:
    #    prev_dim = self.item_layer[-1]
    # elif self.item_layer !=[] and self.user_layer == []:
    #    prev_dim = self.item_layer[-1] + self.user_inputsize
    # elif self.item_layer==[] and self.user_layer != []:
    #    prev_dim = self.item_inputsize + self.user_layer[-1]
    # else:
    #    prev_dim = self.item_inputsize + self.user_inputsize
    if user_layer != []:
      self.user_embeddings = PreNormResidualBlock(self.main_layer_sizes[0])
      # self.user_embeddings = nn.Sequential(
      #     nn.LayerNorm(self.user_layer[-1]),
      #     nn.Linear(self.user_layer[-1], main_layer_sizes[0], bias=False),
      # )
    ...
    if item_layer != []:
      self.item_embeddings = PreNormResidualBlock(self.main_layer_sizes[0])
    else:
      self.item_embeddings = nn.Sequential(
          nn.LayerNorm(self.item_inputsize),
          nn.Linear(self.item_inputsize, main_layer_sizes[0], bias=False),
    ...
    layers = [nn.PReLU()]  
    prev_dim = self.main_layer_sizes[0]
    for out_dim in self.main_layer_sizes[1:-1]:
        layers.extend([
            PreNormResidualBlock(prev_dim) 
        ])
