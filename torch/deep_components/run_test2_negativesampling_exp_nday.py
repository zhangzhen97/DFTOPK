...
if __name__ == '__main__':
   ...
   def run_eval():
        ...
        with open("./deep_components/file_1st.txt", mode='r') as f:
            lines = f.readlines()
            for line in lines:
                eval_day_list.append(line)
        eval_day_list.append("2024-02-04")
        print("eval_day_list:%s,%s" % (len(eval_day_list), eval_day_list) )
        ...
        print("testing files:")
        print(f"number of train_csv: {len(path_to_eval_csv_lst)}")
        ...
        for n_day in range(1, len(eval_day_list)): 
            current_day = eval_day_list[n_day]

            print(f"Loading models for day[{n_day}]: {current_day}")
            if args.epochs > 1:
                prefix = "/checkpoints/E%s" % (args.epochs)
            else:
                prefix = "/checkpoints/"
            ...
            path_to_save_model = root_path + prefix + f"prerank_tau-{args.loss_type}-{args.tau}--bs-{args.batch_size}_lr-{args.lr}_{args.tag}-SN{args.sample_num}_D{n_day - 1}.pkl"
            print("JOINT. Loading prerank model[%s] from:%s" % (n_day, path_to_save_model))
            state_dict = torch.load(path_to_save_model)
            prerank_model.load_state_dict(state_dict)

            path_to_save_model = root_path + prefix + f"retrival_tau-{args.loss_type}-{args.tau}--bs-{args.batch_size}_lr-{args.lr}_{args.tag}-SN{args.sample_num}_D{n_day -1 }.pkl"
            print("JOINT. Loading retrival model[%s] from:%s" % (n_day, path_to_save_model))
            state_dict = torch.load(path_to_save_model)
            retrival_model.load_state_dict(state_dict)

            print("Switch to eval mode (for BatchNorm etc.)")
            prerank_model.eval()
            retrival_model.eval()
            print("Evaluating on next day data:%s"%n_day)
            ...
            evaluate_join_negativesampling(prerank_model, retrival_model, loader, device,160, desc="%s\t%s\tD%s" % (args.loss_type, "joint", n_day))
        ...
        for n_day in range(1, len(eval_day_list)): 
            ...
            path_to_save_model = root_path + prefix + f"prerank_tau-{args.loss_type}-{args.tau}--bs-{args.batch_size}_lr-{args.lr}_{args.tag}-SN{args.sample_num}_D{n_day - 1}.pkl"
            print("SINGLE. Loading prerank model[%s] from:%s" % (n_day, path_to_save_model))
            state_dict = torch.load(path_to_save_model)
            prerank_model.load_state_dict(state_dict)

            path_to_save_model = root_path + prefix + f"retrival_tau-{args.loss_type}-{args.tau}--bs-{args.batch_size}_lr-{args.lr}_{args.tag}-SN{args.sample_num}_D{n_day - 1}.pkl"
            print("SINGLE. Loading retrival model[%s] from:%s" % (n_day, path_to_save_model))
            state_dict = torch.load(path_to_save_model)
            retrival_model.load_state_dict(state_dict)
            ...
            for model, name in [[prerank_model, "prerank"], [retrival_model, "retrival"]]:
                ...
                evaluate_negativesampling(model, loader, device,160, is_debug=False, desc="%s\t%s\tD%s" % (args.loss_type, name, n_day))
run_eval()
